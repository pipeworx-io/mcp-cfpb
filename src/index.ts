...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'CFPB');
}
...
async function searchComplaints(
  ...
  if (!res.ok) throw await httpError(res, 'CFPB API error');
...
/**
 * CFPB's `company` filter is an EXACT match on its own registered spelling, and
 * nobody types that spelling. "WELLS FARGO" returns 0 complaints; the real value
 * is "WELLS FARGO & COMPANY", which has 171,948. A confident zero over six
 * figures of data is the most trust-destroying answer this pack can give — worse
 * than an error, because nothing looks broken.
 *
 * CFPB publishes the resolver itself (_suggest_company), so use it rather than
 * guessing at suffixes. Called only when the literal name finds nothing, so an
 * already-correct name costs no extra request.
 */
async function suggestCompany(company: string): Promise<string | null> {
  try {
    const url = `${BASE_URL}_suggest_company/?text=${encodeURIComponent(company)}`;
    const res = await pwFetch(url);
    if (!res.ok) return null;
    const names = await res.json();
    if (!Array.isArray(names)) return null;
    const first = names.find((n): n is string => typeof n === 'string' && n.trim().length > 0);
    return first && first.toUpperCase() !== company.toUpperCase() ? first : null;
  } catch {
    return null; // resolution is a bonus; never turn a working query into an error
  }
}

async function fetchCompanyPage(company: string, size: number) {
...
async function companyComplaints(company: string, limit?: number) {
  ...
  let data = await fetchCompanyPage(company, size);
  let matchedVia: 'exact' | 'cfpb_suggest' = 'exact';

  if (totalOf(data) === 0) {
    const suggestion = await suggestCompany(company);
    if (suggestion) {
      const retry = await fetchCompanyPage(suggestion, size);
      if (totalOf(retry) > 0) {
        resolved = suggestion;
        data = retry;
        matchedVia = 'cfpb_suggest';
      }
    }
  }

  return {
    // Always say WHAT WAS ACTUALLY QUERIED. A resolver that silently substitutes
    // a different entity is how you answer confidently about the wrong company.
    company: resolved,
    ...
    complaints: (data.hits?.hits ?? []).map(formatComplaint),
    ...(totalOf(data) === 0
      ? { note: `CFPB holds no complaints filed under "${company}", and its company-name suggester offered no match. CFPB matches its own registered legal name exactly (e.g. "WELLS FARGO & COMPANY", not "Wells Fargo"). Try cfpb_top_companies to see the spellings CFPB actually uses.` }
      : {}),
  };
}

/** CFPB returns total as either a number or {value, relation}. */
function totalOf(data: CfpbResponse): number {
  const total = (data.hits as { total?: unknown } | undefined)?.total;
...
async function getComplaint(complaintId: string) {
  const res = await pwFetch(`${BASE_URL}${encodeURIComponent(complaintId)}`);
...
async function productBreakdown(company?: string, startDate?: string, endDate?: string) {
  let resolved = company;
  let matchedVia: 'exact' | 'cfpb_suggest' = 'exact';
  let data = await fetchProductBreakdown(company, startDate, endDate);

  if (company && totalOf(data) === 0) {
    const suggestion = await suggestCompany(company);
    if (suggestion) {
      const retry = await fetchProductBreakdown(suggestion, startDate, endDate);
      if (totalOf(retry) > 0) {
        resolved = suggestion;
        matchedVia = 'cfpb_suggest';
        data = retry;
  ...
  return {
    filters: {
      ...
      company_requested: company ?? null,
      matched_via: company ? matchedVia : null,
      date_range: { start: startDate ?? null, end: endDate ?? null },
    },
    total_complaints: totalOf(data),
...
async function fetchProductBreakdown(company?: string, startDate?: string, endDate?: string) {
  const params = new URLSearchParams({ size: '0', field: 'all' });
